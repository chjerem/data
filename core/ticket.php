...
if($_POST['modify']||$_POST['quit']||$_POST['mail']||$_POST['upload']||$save=="1"||$_POST['send']||$_POST['action']) 
{
	...
	if (($_GET['action']=='new') && ($error=="0"))
	{
		...
		//insert ticket
		$db->exec("INSERT INTO tincidents (user,type,u_group,u_service,u_agency,sender_service,technician,t_group,title,description,date_create,date_hope,date_res,priority,criticality,state,creator,time,time_hope,category,subcat,object,techread,techread_date,place,asset_id,start_availability,end_availability,availability_planned) VALUES ('$_POST[user]','$_POST[type]','$u_group','$u_service','$_POST[u_agency]','$_POST[sender_service]','$_POST[technician]','$t_group',$_POST[title],$_POST[description],'$_POST[date_create]','$_POST[date_hope]','$_POST[date_res]','$_POST[priority]','$_POST[criticality]','$_POST[state]','$_SESSION[user_id]','$_POST[time]','$_POST[time_hope]','$_POST[category]','$_POST[subcat]','$_POST[object]','$techread','$techread_date','$_POST[ticket_places]','$_POST[asset_id]','$start_availability','$end_availability','$_POST[availability_planned]')");
	...
	if($_POST['mail'])
	{
		...
		$www = "./index.php?page=preview_mail&id=$_GET[id]&userid=$_GET[userid]&state=$_GET[state]&category=$_GET[category]&subcat=$_GET[subcat]&object=$_GET[object]&viewid=$_GET[viewid]&view=$_GET[view]&date_start=$_GET[date_start]&date_end=$_GET[date_end]";
		echo '
