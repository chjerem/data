...
if ($_GET['action']=="update") 
{
	...
	}elseif($_GET['table']=='tsubcat') //special case object
	{
		//secure string
		$_POST['object']=strip_tags($_POST['object']);
		
		$qry=$db->prepare("UPDATE `tobject` SET `subcat`=:subcat, `name`=:name WHERE id=:id");
		$qry->execute(array(
			'subcat' => $_POST['subcat'],
			'name' => $_POST['object'],
			'id' => $_GET['id']
...
if ($_GET['action']=="add")
{
	...
	}elseif($_GET['table']=='tobject') //special case subcat 
	{
		//secure string
		$_POST['object']=strip_tags($_POST['object']);
		
		$qry=$db->prepare("INSERT INTO `tobject` (`subcat`,`name`) VALUES (:subcat,:name)");
		$qry->execute(array(
			'subcat' => $_POST['subcat'],
			'name' => $_POST['object']
			));
...
<div class="tabbable tabs-left">
	<ul class="nav nav-tabs" id="myTab3">
		...
		if($rright['admin_lists_subcat']!=0 || $rright['admin']!=0)
		{
			...
			';
            echo '
			<li '; if($_GET['table']=='tobject') {echo 'class="active"';} echo' >
				<a href="./index.php?page=admin&amp;subpage=list&amp;table=tobject">
					<i class="blue icon-table bigger-110"></i>
					'.T_('Objet').'
				</a>
			</li>
							...
							<div class="widget-body">
								<div class="widget-main no-padding">
									...
									';
                                        ...
                                        elseif($_GET['table']=='tobject')
										{
												//find value
												$qry = $db->prepare("SELECT `id`,`name`,`subcat` FROM `tobject` WHERE `id`=:id");
												$qry->execute(array(
													'id' => $_GET['id']
													));
												$row=$qry->fetch();
												...
												//find category name
												$qry = $db->prepare("SELECT `id` FROM `tsubcat` WHERE `id`=:id");
												$qry->execute(array(
													'id' => $row['cat']
													));
												$rowcatfind=$qry->fetch();
												...
												echo '
													<fieldset>
														<label for="subcat">'.T_('Sous-Catégorie').'</label>
														<select name="subcat" id="form-field-select-1">
														';
															if($rright['dashboard_service_only']!=0 && $rparameters['user_limit_service']==1) {
																//display only category associated services of this current user
																$qry = $db->prepare("SELECT `tsubcat.id`,`tsubcat.name` FROM `tsubcat` WHERE `tcategory.service` IN (SELECT `service_id` FROM `tusers_services` WHERE `user_id`=:user_id) ORDER BY `tcategory.name`");
																$qry->execute(array(
																	'user_id' => $_SESSION['user_id']
																	));
															} else {
														...
														<div class="space-4"></div>
														<label for="object">'.T_('Object').'</label>
														<input name="object" type="text" value="'.$row['name'].'" />
													</fieldset>
												';
										} 
										...
										}elseif($_GET['table']=='tobject') //special case subcat 
										{
											...
											<fieldset>
												...
												';
													if($rright['dashboard_service_only']!=0 && $rparameters['user_limit_service']==1) {
														//display only category associated services of this current user
														$qry = $db->prepare("SELECT `tsubcat.id`,`tsubcat.name` FROM `tsubcat` WHERE `tsubcat.service` IN (SELECT `service_id` FROM `tusers_services` WHERE user_id=:user_id) ORDER BY `tsubcat.name`");
														$qry->execute(array(
															'user_id' => $_SESSION['user_id']
															));
													} else {
