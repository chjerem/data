...
if($_POST['addiface'])
{
	$db->exec("INSERT INTO tassets_iface (role_id,asset_id,netbios,ip,mac,disable) VALUES ($_POST[role],$_GET[id],'','','','0')");
	//redirect
...
if($_POST['editiface'])
{
	$db->exec("UPDATE tassets_iface SET role_id=$_POST[role] WHERE id=$_GET[iface]");
	//redirect
	$www = "./index.php?page=asset&id=$_GET[id]&$url_get_parameters";
	echo '<script language="Javascript">
	<!--
...
{
	//get current iface
	$query=$db->query("SELECT role_id FROM tassets_iface WHERE id=$_GET[iface]");
	$role_id=$query->fetch();
	$query->closeCursor();
